...
install :; forge install --no-commit smartcontractkit/chainlink-brownie-contracts && forge install --no-commit rari-capital/solmate && forge install --no-commit foundry-rs/forge-std
...
deploy-all :; make deploy-${network} contract=APIConsumer && make deploy-${network} contract=KeepersCounter && make deploy-${network} contract=PriceFeedConsumer && make deploy-${network} contract=SquadGame
